'''
	This file is noy to be directly executed.
	It is used in other files indirectly, so it is needed in its directory. 
'''
'''
Ignore this: For our reference.

Rates : Whole
Period : ' ','herein.^l'
Attaching_To_Delegated... : Whole
Insured : ' ','and/or^s'
Interest : 'consisting^s','handled^l'
Unique_Market_Reference : Whole
Type : Whole
'''

def EntityEx(string , flag): 
	flag = flag[1:-1]
	t = flag.partition(',')
	start = t[0].strip('\'')
	end = t[2].strip('\'')
